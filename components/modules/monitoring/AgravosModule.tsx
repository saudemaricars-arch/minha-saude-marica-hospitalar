...
import { NotificationCase } from '../../../types';
...
const AgravosModule: React.FC<AgravosModuleProps> = ({ onBack }) => {
  ...
  const [notifications, setNotifications] = useState<NotificationCase[]>(MOCK_NOTIFICATIONS);

  // Form State
  const [newCase, setNewCase] = useState({ disease: '', patientName: '' });

  const handleSave = () => {
      const caseToAdd: NotificationCase = {
          id: Date.now().toString(),
          disease: newCase.disease,
          patientName: newCase.patientName,
          notificationDate: new Date().toLocaleDateString('pt-BR'),
          status: 'suspeito',
          week: 20, // Mock current week
          investigationStatus: 'em_andamento'
      };
      setNotifications([caseToAdd, ...notifications]);
      setView('list');
