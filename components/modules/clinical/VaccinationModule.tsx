...
const VaccinationModule: React.FC<VaccinationModuleProps> = ({ onBack }) => {
    ...
    const loadPatients = async () => {
        try {
            const data = await vaccinationService.fetchPatients();
            // Calculate mock stats for display since DB is simple
            const processed = data.map(p => ({
                ...p,
                age: calculateAge(p.birth_date),
                complianceRate: Math.floor(Math.random() * 30) + 70, // Mock calc
                delayedVaccines: Math.floor(Math.random() * 2), // Mock calc
                records: p.records || []
            }));
                                ...
                                {patients.map(p => (
                                    <tr key={p.id} className="hover:bg-teal-50/30 transition-colors cursor-pointer" onClick={() => setSelectedPatient(p)}>
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-xs text-gray-500">Nasc: {p.birth_date}</div>
                                        </td>
                                ...
                                <div className="space-y-3">
                                    {selectedPatient.records?.map((rec, i) => (
                                        <div key={i} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm flex items-center justify-between">
                                            <div>
                                                <p className="font-bold text-gray-900">{rec.vaccine_name}</p>
                                                <p className="text-sm text-gray-500">{rec.dose}</p>
                                            </div>
                                            <div className="text-right">
                                                {getStatusBadge(rec.status)}
                                                <p className="text-xs text-gray-500 mt-1">
                                                    {rec.status === 'Applied' ? `Aplicado em: ${rec.date_applied}` : `Agendado para: ${rec.date_scheduled}`}
                                                </p>
