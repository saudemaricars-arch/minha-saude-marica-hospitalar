...
import { NeonatalPage, NeonatalPatient, TestStatus } from '../../../types';
...
const NeonatalScreeningModule: React.FC<NeonatalScreeningModuleProps> = ({ onBack }) => {
  ...
  const [patients, setPatients] = useState<NeonatalPatient[]>(MOCK_NEONATAL_PATIENTS);
  ...
  const handleSave = (e: React.FormEvent) => {
      e.preventDefault();
      if (!newBaby.name || !newBaby.motherName) return;

      const patient: NeonatalPatient = {
          id: Math.random().toString(36).substr(2, 9),
          name: newBaby.name,
          motherName: newBaby.motherName,
          birthDate: newBaby.birthDate || new Date().toLocaleDateString('pt-BR'),
          birthTime: newBaby.birthTime || '12:00',
          gestationalAge: newBaby.gestationalAge || '39s',
                        ...
                        {patients.map(patient => (
                            <tr key={patient.id} className="hover:bg-pink-50/30 transition-colors cursor-pointer" onClick={() => setSelectedPatient(patient)}>
                                <td className="px-6 py-4">
                                    ...
                                    <div className="text-xs text-gray-500">Mãe: {patient.motherName}</div>
                                </td>
                                <td className="px-6 py-4">
                                    <div>{patient.birthDate}</div>
                                    <div className="text-xs text-gray-500">{patient.birthTime} • {patient.weight}g</div>
                                </td>
                        ...
                        <div key={patient.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">Mãe: {patient.motherName} • Tel: (21) 99999-9999</p>
                                </div>
                            </div>
                    ...
                    <div className="bg-gray-900 text-white p-6 flex justify-between items-start">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="opacity-80 text-sm">Mãe: {selectedPatient.motherName}</p>
                            </div>
                        </div>
                    ...
                    <div className="p-6 bg-gray-50 border-b border-gray-200">
                        <div className="grid grid-cols-3 gap-4 text-center">
                            <div>
                                ...
                                <p className="font-medium text-gray-900">{selectedPatient.birthDate} às {selectedPatient.birthTime}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p className="font-medium text-gray-900">{selectedPatient.gestationalAge}</p>
                            </div>
                        </div>
                    </div>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input 
                    ...
                    required 
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none"
                    value={newBaby.motherName || ''}
                    onChange={e => setNewBaby({ ...newBaby, motherName: e.target.value })}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                        type="time" 
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none"
                        value={newBaby.birthTime || ''}
                        onChange={e => setNewBaby({ ...newBaby, birthTime: e.target.value })}
                    />
                  </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                        ...
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 outline-none"
                        value={newBaby.gestationalAge || ''}
                        onChange={e => setNewBaby({ ...newBaby, gestationalAge: e.target.value })}
                    />
                  </div>
              </div>
