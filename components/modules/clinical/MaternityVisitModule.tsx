...
import { Icons, MOCK_MATERNITY_VISITS } from '../../../constants';
import { MaternityPage, MaternityVisit } from '../../../types';
...
const MaternityVisitModule: React.FC<MaternityVisitModuleProps> = ({ onBack }) => {
  const [activeTab, setActiveTab] = useState<MaternityPage>('visits');
  const [isVisitModalOpen, setIsVisitModalOpen] = useState(false);
  const [selectedVisit, setSelectedVisit] = useState<MaternityVisit | null>(null);

  // Helper Stats
  const totalInpatients = MOCK_MATERNITY_VISITS.length;
  const pendingVisits = MOCK_MATERNITY_VISITS.filter(v => v.status === 'pendente').length;
  const highRisk = MOCK_MATERNITY_VISITS.filter(v => v.riskLevel === 'Alto').length;
            ...
            <div className="space-y-6 animate-fade-in">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div className="space-y-3">
                        {MOCK_MATERNITY_VISITS.filter(v => v.alerts && v.alerts.length > 0).map(v => (
                            <div key={v.id} className="flex items-start gap-4 p-3 bg-rose-50 border border-rose-100 rounded-lg">
                                ...
                                <div>
                                    <p className="font-bold text-sm text-gray-900">{v.motherName} - Leito {v.room}</p>
            ...
            <div className="animate-fade-in bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-100 text-sm">
                        {MOCK_MATERNITY_VISITS.map(visit => (
                            <tr key={visit.id} className="hover:bg-rose-50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <div className="font-medium text-gray-900">{visit.motherName}</div>
                                    <div className="text-xs text-gray-500">{visit.babyName}</div>
                                </td>
                                <td className="px-6 py-4">
                                    <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs font-bold">
                                        {visit.daysPostPartum}º Dia ({visit.type})
                                    </span>
                                </td>
                                <td className="px-6 py-4">
                                    <span className={`text-xs font-bold px-2 py-1 rounded ${
                                        visit.riskLevel === 'Alto' ? 'bg-red-100 text-red-700' :
                                        visit.riskLevel === 'Médio' ? 'bg-yellow-100 text-yellow-700' :
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
                <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full h-[90vh] flex flex-col overflow-hidden">
                    <div className="bg-rose-600 text-white p-6 flex justify-between items-start flex-shrink-0">
                        <div>
                            ...
                            <p className="opacity-90 text-sm mt-1">
                                {selectedVisit.motherName} & {selectedVisit.babyName} • Leito {selectedVisit.room}
                            </p>
                        </div>
                    ...
                    <div className="p-6 bg-white border-t border-gray-200 flex justify-end gap-3 flex-shrink-0">
                        ...
                        <button className="px-6 py-2 bg-rose-600 text-white rounded-lg font-bold shadow hover:bg-rose-700">
                            Salvar Registro
                        </button>
                    </div>
                </div>
