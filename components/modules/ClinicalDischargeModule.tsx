        ...
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex bg-white p-1 rounded-lg border border-gray-200 shadow-sm overflow-x-auto">
             ...
             <button 
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === 'indicators' ? 'bg-green-50 text-green-700 shadow-sm' : 'text-gray-600 hover:bg-gray-50'}`}
             >
            ...
            <div className="animate-fade-in space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <p className="text-2xl font-bold text-gray-900">{MOCK_DISCHARGES.filter(d => d.status === 'em_preparo').length}</p>
                    </div>
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                         <div className="flex justify-between items-center mb-2">
                         ...
                         <p className="text-2xl font-bold text-gray-900">{MOCK_DISCHARGES.filter(d => d.transportRequired && d.status !== 'liberado').length}</p>
                    </div>
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                         <div className="flex justify-between items-center mb-2">
                         ...
                         <p className="text-2xl font-bold text-gray-900">{MOCK_DISCHARGES.filter(d => d.status === 'liberado').length}</p>
                    </div>
                </div>
                                ...
                                <tr key={discharge.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${
                                            discharge.status === 'liberado' ? 'bg-green-100 text-green-700' :
