...
const Dashboard: React.FC<DashboardProps> = ({ onLogout, currentUnit, onSwitchUnit }) => {
  ...
  const filteredModules = React.useMemo(() => {
    ...
    let modulesToFilter = MODULES_DATA;

    // IF MASTER: Dynamically generate modules for ALL units + Global Dashboard
    if (isMasterUser) {
      // Start with the Global Dashboard (ID 99)
      const globalModule = MODULES_DATA.find(m => m.id === '99');

      // Generate modules for each unit
      const unitModules = HEALTH_UNITS
        .filter(u => u.id !== 'master') // Exclude the master unit itself from the list
        .map(unit => {
          // Determine icon based on unit type
          let iconName = 'Building';
          if (unit.type === 'Maternidade') iconName = 'Baby';
          if (unit.type === 'Laboratório') iconName = 'TestTube';
          if (unit.type === 'USF') iconName = 'Activity';
          if (unit.type === 'Emergência') iconName = 'Siren';
          if (unit.type === 'CAPS') iconName = 'Smile'; // Or Brain if available, using Smile for mental health context

          return {
            id: `master_unit_${unit.id}`,
            title: `Análise: ${unit.name}`,
            count: 0, // Could be dynamic later
            category: 'master_analysis' as any,
            iconName: iconName
          };
        });

      // Combine Global + Dynamic Units (User requested ONLY units and global dashboard)
      // Reverting to strict logic: Hide standard modules for Master
      modulesToFilter = globalModule ? [globalModule, ...unitModules] : unitModules;
    }

    return modulesToFilter.filter(m => {
      // 2. Filter Logic

      // Strict Category Filtering for Master User (Restored)
      if (isMasterUser) {
  ...
  }, [currentUnit, searchTerm, activeFilter]);
